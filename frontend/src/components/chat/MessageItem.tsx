...
const MessageItem = ({message, index, messages, selectedConvo, lastMessageStatus}:MessageItemProps) => {
    ...
    >
        ...
        <div
            className={cn("max-w-xs lg:max-w-md flex flex-col",
                message.isOwn ? "items-end" : "items-start"
            )}
        >
            <Card
                className={cn("p-3", message.isOwn ? "chat-bubble-sent border-9": "bg-chat-bubble-received")}
            >
            ...
            {message.isOwn && message._id === selectedConvo.lastMessage?._id &&(
                <Badge
                    ...
                    className={cn(
                        ...
                        lastMessageStatus === 'seen' ? "bg-primary/20" : "bg-muted text-muted-foreground"
                    )}
                >
                    {lastMessageStatus}
                </Badge>
